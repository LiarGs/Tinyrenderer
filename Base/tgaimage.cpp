...
bool TGAImage::read_tga_file(const std::string& filename)
{
    ...
    {
        LOGE("bad bpp (or width/height) value");
        return false;
    }
    // 分配内存并读取图像数据
    ...
    {
        // 如果 datatypecode 是 10 或 11（代表 RLE 压缩的 RGB 或灰度数据），则读取 RLE 压缩的图像数据
        if (!load_rle_data(in))
        {
            LOGE("an error occured while reading the data");
            return false;
    ...
    {
        LOGE("unknown file format %d", (int)header.datatypecode);
        return false;
    }
    // 检查图像描述符
    // TGA 文件包含一个 imagedescriptor 字段，用来指示图像的垂直和水平翻转
    // 根据该字段的标志位，决定是否执行翻转
    constexpr uint8_t VERTICAL_FLIP_FLAG = 0x20;
    constexpr uint8_t HORIZONTAL_FLIP_FLAG = 0x10;
    if (!(header.imagedescriptor & VERTICAL_FLIP_FLAG))
        flip_vertically();
    if (header.imagedescriptor & HORIZONTAL_FLIP_FLAG)
        flip_horizontally();
    LOGI("%dx%d/%d", w, h, bpp * 8);
...
bool TGAImage::write_tga_file(const std::string& filename, const bool vflip, const bool rle) const
{
    ...
    if (!out.is_open())
    {
        LOGE("can't open file %s", filename.data());
        return false;
    }
    // 设置 TGA 文件头
    TGAHeader header = {};
    ...
    else if (!unload_rle_data(out)) // 使用 RLE 压缩
    {
        LOGE("can't unload rle data");
        return false;
    }
    // 写入开发者区和扩展区
